...
true.ar=c(0.8,rep(0,4),0.7,-0.56,rep(0,7)) #True Autoregressive Coefficietns
true.ma=rep(0,14) #True Moving Average Coefficients
coef.true=c(true.ar,true.ma) #Vector of True AR and MA Coefficients
...
for(l in 1:S){
  for(k in 1:length(noise)){
    for(j in 1:length(N)){
      ...
      ####################################################################################################################################
      #Simulate Dataset
      model<-Arima(ts(rnorm(N[j]+1000,0,1),freq=6),order=c(1,0,0),seasonal=c(1,0,0),include.mean=F,fixed=c(0.8,0.7))
      ...
      save.image("ADLASSOSIM1AICBICB.Rdata")
